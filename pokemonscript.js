...
let getPokeData = () => {
  ...
  // Leere den Kartencontainer, um alte Karten zu entfernen
  cardContainer.innerHTML = '';

  // Generiere zwei zufällige IDs für Pokémon
  let ids = [Math.floor(Math.random() * 250) + 1, Math.floor(Math.random() * 1000) + 1, Math.floor(Math.random() * 1000) + 1,
    Math.floor(Math.random() * 1000) + 1, Math.floor(Math.random() * 1000) + 1, Math.floor(Math.random() * 1000) + 1];

  // Fetch die Daten für beide Pokémon
  ids.forEach(id => {
